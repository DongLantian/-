...
7. 接口中的属性都是public static final的，必须初始化；抽象类中的属性和普通的类没有区别。
...
2. 懒汉式
   ...
   ​	上述加锁方式仍然不能保证安全。首先，由于指令会重排序，instance变量有可能再未被初始化的情况下被其他线程判断非空后使用，造成空指针异常；其次，第一次null判断后线程阻塞，另一个线程创建实例后第一个线程再次创建实例造成多个实例被创建，因此，又有了双重校验锁模式。
